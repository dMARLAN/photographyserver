...
async def __get_existing_photos_by_path(session: AsyncSession) -> dict[str, PLPhoto]:
...
async def sync_filesystem_to_db(session: AsyncSession, storage_path: Path | str | None = None) -> SyncStats:
    """Scan storage directory and update database with photo metadata.

    This function:
    1. Scan the storage directory for image files
    ...
    Args:
        ...
        storage_path: Path to the storage directory. If None, use a configured photos path.
...
async def remove_orphaned_photos(session: AsyncSession) -> int:
